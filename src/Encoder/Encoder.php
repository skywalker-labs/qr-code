...
namespace Skywalker\QrCode\Encoder;

use Skywalker\QrCode\Common\BitArray;
use Skywalker\QrCode\Common\CharacterSetEci;
...
use Skywalker\QrCode\Exception\WriterException;
use SplFixedArray;
...
final class Encoder
{
    ...
    public static function encode(
        ...
        switch ($mode->getBits()) {
            case Mode::BYTE()->getBits():
                $numLetters = $dataBits->getSizeInBytes();
                break;

            case Mode::NUMERIC()->getBits():
            case Mode::ALPHANUMERIC()->getBits():
                $numLetters = strlen($content);
                break;

            case Mode::KANJI()->getBits():
                $numLetters = iconv_strlen($content, 'utf-8');
                break;

            default:
                throw new WriterException('Invalid mode');
        }
